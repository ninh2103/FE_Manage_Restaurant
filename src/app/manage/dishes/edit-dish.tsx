...
export default function EditDish({
  ...
  const { data } = useGetDish({ enabled: Boolean(id), id: id as number });
  ...
  useEffect(() => {
    if (data) {
      const { name, image, description, status, price } = data.payload.data;
      form.reset({
        name,
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[600px] max-h-screen overflow-auto">
        ...
        <DialogFooter>
          <Button type="submit" form="edit-dish-form" onClick={handleEdit}>
            Lưu
          </Button>
        </DialogFooter>
      </DialogContent>

...
export default function OrderGuestDetail({
  ...
  const purchasedOrderFilter = guest
    ...
    : [];
  const payGuestMutation = usePayForGuestMutation();
  const pay = async () => {
    if (payGuestMutation.isPending || !guest) return;
    try {
      const result = await payGuestMutation.mutateAsync({ guestId: guest.id });
